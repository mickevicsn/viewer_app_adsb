...
async function main() {
  console.info(`ADS-B viewer build: ${VIEWER_BUILD_ID}`);
  ...
  const hud = new DashboardUI(state, {
    onReset: async () => {
      ...
      const isPresentationMode = state.appMode === APP_MODE_PRESENTATION;

      let preset = null;
      if (isPresentationMode) {
        // In presentation flow, Reset first asks for the preset so the user can
        // choose the timeline/camera setup before choosing another method for
        // the locked 4BAAD9 flight.
        preset = await showPresetChooser({
          title: "Choose presentation preset",
          message: "Select the start time, speed, and camera setup before choosing the reconstruction method.",
        });
      }

      const chooserFlights = isPresentationMode
